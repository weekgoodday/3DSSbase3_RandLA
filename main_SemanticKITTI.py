...
# ！！这个dataloader的方式不太一样 问：每个batch的数据是怎样的？
...
def train_one_epoch():
    ...
    for batch_idx, batch_data in enumerate(TRAIN_DATALOADER):
        ...
        total_acc=total_acc*(batch_idx)/(batch_idx+1)+acc/(batch_idx+1)
        total_loss=total_loss*(batch_idx)/(batch_idx+1)+loss.item()/(batch_idx+1)
        # Accumulate statistics and print out
        for key in end_points:
            if 'loss' in key or 'acc' in key or 'iou' in key: #有loss和acc
...
def train(start_epoch):
    ...
    for epoch in range(start_epoch, FLAGS.max_epoch):
        ...
        torch.save(save_dict, os.path.join(LOG_DIR, 'checkpoint_train.tar'))
